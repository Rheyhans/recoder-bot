...
ownserver = setting.OwnServer
designer = setting.Designer
coder = setting.Coder
...
const isListMessage = (type == 'listResponseMessage') ? mek.message.listResponseMessage.title : ''
...
sendButMessage(from, ` *VIRTEX DETECTOR*\n\nMaaf Kamu Akan Di Kick Dari Group Ini!`, `Klik Di Bawah Untuk Mematikan`, [
{
buttonId: `${prefix}anvirtex disable`,
buttonText: {
displayText: `Disable Antivirtex🌱`,
},
type: 1,
}]);
Ryuu.groupRemove(from, [sender])
...
Ryuu.sendMessage(from, sendResponList(from, body, db_respon_list), MessageType.text, {
quoted: mek
})
} else {
Ryuu.sendMessage(from, await getBuffer(get_data_respon.image_url), MessageType.image, {
quoted: mek,
...
reqXp  = 5000 * (Math.pow(2, getLevelingLevel(sender)) - 1)
var p = await Ryuu.getStatus(`${sender}`, MessageType.text)
ptod = "6285895001126@s.whatsapp.net"
stod = `${sender}`
dtod = 'ngetes'
uwu = '```'
...
│✾ _*Limit*_ : ${getLimit(sender, limitCount, limit)}/${limitCount}
╰───`
Sendbutdocument(from, tescok, `Runtime : ${runtime(process.uptime())}`, fs.readFileSync('./media/Ryuu.pdf'), { mimetype: Mimetype.pdf, thumbnail: fs.readFileSync('./media/Ryuu.jpg'), filename: `Rhey.pdf`, pageCount: 999 }, [{ buttonId: `${prefix}listmenu`, buttonText: { displayText: 'LIST MENU 🌱' }, type: 1 }, { buttonId: `${prefix}allmenu`, buttonText: { displayText: 'ALL MENU ⛩️' },type: 1 }, { buttonId: `${prefix}owngtps`, buttonText: { displayText: 'OWN GTPS 😁' }, type: 1 },{ buttonId: `${prefix}store`, buttonText: { displayText: 'OWN STORE 😉' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [ptod,dtod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} ${pushname}\nScript Create By RyuukaBotz\nRemaker By Rhey`,mediaType:"2",thumbnail:fakeimage,mediaUrl:`https://www.youtube.com/watch?v=GhTn-9p__EQ`}}})
break
case 'allmenu':
if (isBanned) return reply('Kamu Sudah Di banned!')
reqXp  = 5000 * (Math.pow(2, getLevelingLevel(sender)) - 1)
...
teks =
`${botname} | Runtime : ${runtime(process.uptime())}\nOwner : ${ownername}`
Ryuu.sendMessage(from, { contentText: `${menu}`+'\n\n'+allmenu(prefix), footerText: `${teks}`, buttons: [{ buttonId: `${prefix}donasi`, buttonText: { displayText: 'DONASI 💵' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER 🌱' }, type: 1 }, {buttonId:`${prefix}owngtps`,buttonText:{displayText:'OWN GTPS 😁'},type:1}], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'othermenu':
if (isBanned) return reply('Kamu Sudah Di banned!')
...
reply('Succes Antivirtex Telah Nyala🌱')
} else if (args[0].toLowerCase() === 'disable'){
let anu = antivirtex.indexOf(from)
antivirtex.splice(anu, 1)
fs.writeFileSync('./database/antivirtex.json', JSON.stringify(antivirtex))
reply('Succes Antivirtex Telah Mati🍂')
} else {
reply(`Pilih enable atau disable`)
}
break
...
.catch((err) => reply(jsonformat(err)))
break
case 'open': case 'groupopen': case 'grupopen':
if (isBanned) return reply('Kamu Sudah Di banned!')
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins && !isOwner) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
...
break
case 'close': case 'groupclose': case 'grupclose':
if (isBanned) return reply('Kamu Sudah Di banned!')
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins && !isOwner) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(`Jadikan Bot Sebagai Admin Group!`)
...
case 'store':
if (isBanned) return reply('Kamu Sudah Di banned!')
reply('https://chat.whatsapp.com/D8h93C9h9gwH86yIwRSrSC')
reply(`Join ya kak`)
break
//Fitur Store by Rhey
case 'addlist':
...
Ryuu.sendMessage(from, listMsg, MessageType.listMessage)
break
case 'owngtps': case 'gtps':
if (isBanned) return reply('Kamu Sudah Di banned!')
capti =`*GTFA*

*_Link Host_*
https://www.mediafire.com/file/ul54vlhu9jipd1v/GTFA.txt/file

◆➣*_Info_* ~Server~
*_Server_* by : ~RheyGT~ *_X_* ~Winter~
*_Helper_* By : ${helper}
*_Designer_* By : ${designer}
*_Coder_* By : ${coder} 
  
Untuk price role silahkan ketik ${prefix}list Kalau sudah Join groub owner`
but = [{ buttonId: `${prefix}store`, buttonText: { displayText: 'JOIN GROUP DLO 😋' }, type: 1 }]
...
if (!isGroup) return
if (!isGroupAdmins) return
menu = `*Pesanan sedang di proses*

Di proses jam *${time} WIB*
Di proses tanggal *${tanggal}*

_Pesanan anda sedang di proses harap sabar ..._`
reply(menu)
}
if (budy == 'p') {
if (!isGroup) return
...
if (!isGroup) return
if (!isGroupAdmins) return
menu = `*Pesanan sudah done*

Selesai pada jam *${time} WIB*
Selesai pada tanggal *${tanggal}*

_Pesanan anda sudah done silakan cek ..._`
reply(menu)
}
if (budy == 'd') {
if (!isGroup) return
